<template>
  <div class="profile-page">
    ...
    <div class="footer" @click="logout">退出登录</div>
  </div>
</template>

<script>
import { fetchUserInfo, fetchLoginState } from '@/utils/fetchData';
import { getLoginUserInfo, setLoginUserInfo,removeLoginUserInfo } from '@/utils/storage';
export default {
  data() {
    return {
      items: [
        ...
        { title: '礼品卡', to: '/giftcard', positionY: `${-120 / 37.5}rem` },
        {
          title: '会员福利',
          to: '/vipwelfare',
          positionY: `${-270 / 37.5}rem`
        },
        { title: '地址管理', to: '/address', positionY: '0' },
        {
          title: '账号安全',
          to: '/accountsecurity',
          positionY: `${-356 / 37.5}rem`
        },
        {
          title: '联系客服',
          to: '/customerservice',
          positionY: `${-180 / 37.5}rem`
        },
  ...
  async beforeRouteEnter(to, from, next) {
    ...
    if (!userInfo || !userInfo.name) return next('/login');
    if (userInfo.uId) return next();
    const user = await fetchUserInfo(userInfo.name);
    if (user) {
